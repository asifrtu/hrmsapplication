...
class Employee extends CI_Controller
{
    ...
    public function employeeList()
    {
        ...
        $this->load->view('hrzone/employeeList', $data1);
    }
    ...
    public function EmployeeListing()
    {
        ...
        // die;
        $data['page'] = 'EmployeeDoc';
        $this->load->view('hrzone/allEmployeeListing', $data);
    }
    ...
    public function AddEmployeeQDX()
    {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            if ($last_user_id) {
                ...
                $checkCaledar = $this->AttendanceModel->getSingleRowDate($current_date);
                if (!empty($checkCaledar->calender_date))
                {
                    $c_date = date("Y-m");
                    $current_date = $c_date.'-01';
                    $dataUserAtt = array(
                        'User_Id'       => $last_user_id,
                        'current_date'  => $current_date
                    );

                    $this->AttendanceModel->storeSingleUserAttendence($dataUserAtt);
                }
                else
                {
                ...
                for ($month = 1; $month <= 12; $month++) {
                    for ($day = 1; $day <= 31; $day++) {
                        // Check if the date is valid (e.g., February 30 doesn't exist).
                        if (checkdate($month, $day, date('Y'))) {
                            $currentDate = date('Y-m-d', strtotime(date('Y') . '-' . $month . '-' . $day));
                            $dayOfWeek = date('w', strtotime($currentDate));
        
                            // Check if it's a Sunday (day of the week = 0) or odd Saturday.
                            if ($dayOfWeek == 0 || ($dayOfWeek == 6 && $day % 2 != 0)) {
                                $isWorkday = 'W-OFF'; // Mark as non-workday (W-OFF)
                            } else {
                                $isWorkday = ''; // Mark as workday
                            }
        
                            $dates[] = array(
                                'calender_date'    => $currentDate,
                                'sort_holidayname' => $isWorkday,
                            );
                        }
                    }
                }
        
                // Insert the generated dates into the database.
                $this->AttendanceModel->insert_dates($dates);
    ...
    public function UserProfileImageUpload()
    {
        ...
        $last_user_id = $this->input->post('frmUserId');
        // die;
        $filesCount = count($_FILES['EmpProfileImage']['name']);
        for ($i = 0; $i < $filesCount; $i++) {
            $_FILES['file']['name']     = $_FILES['EmpProfileImage']['name'][$i];
    ...
    public function Birthday()
    {
        ...
        $date =  date("Y-m-d");
        $query = $this->db->get_where('employee_birthday', ['birthday_date' => $date])->result_array();
    ...
    public function EmployeeBirthday()
    {
        ...
        // $uid = 139;
        //$query = $this->db->get_where('employee_birthday', ['user_id'=> $uid])->row()->birthday_date;
        // echo  $this->db->get_where('tbl_user', ['employeeStatus' => $active])->row()->name;    
        ...
        $this->load->view("hrzone/employeebirthday", $data);
    }



    public function EmployeeProfileDeleteById()
    {
        // print_r($_GET);die;
        // print_r($_POST);die;
        // print_r($_GET);die;
        ...
        if (!empty($this->input->post())) {
            ...
            $this->HrObj->EmployeeDetailsDelete($id);
